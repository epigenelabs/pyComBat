...
from utils import model_matrix, all_1
from utils import compute_prior, postmean, postvar, it_sol, int_eprior
from utils import check_mean_only, define_batchmod, check_ref_batch, treat_batches, treat_covariates, check_NAs
from utils import calculate_mean_var, calculate_stand_mean
from utils import standardise_data, fit_model, adjust_data
...
def test_means():
    ...
    for i in range(len(matrix.T)):
        print(np.mean(matrix.T[i]),"\t",np.mean(matrix_adjusted.T[i]))
    print("**********")
    print("var matrix:",np.var(matrix))
    print("var matrix (adjusted):",np.var(matrix_adjusted))
    for i in range(len(matrix.T)):
        print(np.var(matrix.T[i]),"\t",np.var(matrix_adjusted.T[i]))
...
NAs = check_NAs(dat)
B_hat, grand_mean, var_pooled = calculate_mean_var(design, batches, ref, dat, NAs, ref_batch, n_batches, n_array)
...
def test_pycombat():
    ...
    assert abs(np.mean(matrix)-np.mean(matrix_adjusted)) <= np.mean(matrix)*0.05
    assert np.var(matrix_adjusted) <= np.var(matrix)
