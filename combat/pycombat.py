...
class PyCombat(BaseEstimator, TransformerMixin):
    ...
    def check_if_fitted(self):
        ...
        assert self.gamma_star is not None, f"gamma_star (estimated additive batch effect) {error_message}"
        assert self.delta_star is not None, f"delta_star (estimated multiplicative batch effect) {error_message}"
        assert self.batch_design is not None, f"batch_design (information about batches in design matrix) {error_message}"
        assert self.n_batches is not None, f"n_batches (list of batches lengths ) {error_message}"
        assert self.var_pooled is not None, f"var_pooled (standardised mean) {error_message}"
        assert self.stand_mean is not None, f"stand_mean (Variance for each gene and each batch) {error_message}"
        assert self.n_array is not None, f"n_array (total size of dataset) {error_message}"
        assert self.batches is not None, f"batches (list of unique batches) {error_message}"

    def fit(self, data: pd.DataFrame, batch: list, mod: list = [], par_prior: bool = True, prior_plots: bool = False, mean_only: bool = False, ref_batch: int = None, precision: float = None) -> PyCombat:
        """
        ...
        if not NAs:
            B_hat, grand_mean, self.var_pooled = calculate_mean_var(design, self.batches, self.ref, self.data_value, NAs, self.ref_batch, self.n_batches, n_batch, self.n_array)
            self.stand_mean = calculate_stand_mean(grand_mean, self.n_array, design, n_batch, B_hat)
            self.standardised_data = standardise_data(self.data_value, self.stand_mean, self.var_pooled, self.n_array)
            ...
            self.gamma_star, self.delta_star, self.batch_design = fit_model(design, n_batch, self.standardised_data, self.batches, mean_only, par_prior, precision, self.ref_batch, self.ref, NAs)
        else:
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for i, batch_index in enumerate(self.batches):  # for each batch, specific correction
            bayes_data[batch_index] = (bayes_data[batch_index] - np.dot(np.transpose(self.batch_design)[batch_index], self.gamma_star)) / np.transpose(np.outer(np.sqrt(self.delta_star[i]), np.asarray([1]*self.n_batches[i])))

        # renormalise the data after correction:
        # 1. multiply by variance
        # 2. add mean
        bayes_data = np.multiply(np.transpose(bayes_data), np.outer(np.sqrt(self.var_pooled), np.asarray([1]*self.n_array))) + self.stand_mean
        ...
        return pd.DataFrame(bayes_data, columns=self.list_samples, index=self.list_genes)
